...
panel_h = (menu_mode == "load") ? 400 : (menu_mode == "end" ? 260 : 320);
...
draw_rectangle(panel_x + tab_w, panel_y, panel_x + tab_w * 2, panel_y + tab_h, false);
draw_set_color(menu_mode == "end" ? make_color_rgb(180,60,60) : col_tab_idle);
draw_rectangle(panel_x + tab_w * 2, panel_y, panel_x + panel_w, panel_y + tab_h, false);

draw_set_color(col_text);
draw_text(panel_x + tab_w/2, panel_y + tab_h/2, "SAVE");
draw_text(panel_x + tab_w + tab_w/2, panel_y + tab_h/2, "LOAD");
draw_text(panel_x + tab_w*2 + tab_w/2, panel_y + tab_h/2, "AKHIRI");
...
if (menu_mode != "end") {
    ...
    var _end_hover = point_in_rectangle(mouse_gui_x, mouse_gui_y, _end_x1, _end_y1, _end_x2, _end_y2);

    draw_set_color(_end_hover ? make_color_rgb(160,50,50) : make_color_rgb(120,40,40));
    draw_rectangle(_end_x1, _end_y1, _end_x2, _end_y2, false);
    draw_set_color(col_panel_bd);
    draw_rectangle(_end_x1, _end_y1, _end_x2, _end_y2, true);

    draw_set_color(c_white);
    draw_set_halign(fa_center);
    draw_set_valign(fa_middle);
    draw_text((_end_x1+_end_x2)/2, (_end_y1+_end_y2)/2, "Akhiri Permainan Sekarang");
    draw_set_halign(fa_left);
    draw_set_valign(fa_top);

    draw_set_color(col_text_dim);
    draw_set_halign(fa_center);
    draw_text(panel_x + panel_w/2, _end_y2 + 20, "Skor akan langsung dihitung berdasarkan");
    draw_text(panel_x + panel_w/2, _end_y2 + 40, "kondisi saat ini.");
    draw_set_halign(fa_left);
}
...
if (show_confirm) {
    ...
    var _msg = "";
	if (menu_mode == "save") {
	    _msg = "Timpa data di Slot " + string(confirm_slot) + "?";
	} else if (menu_mode == "load") {
	    _msg = "Muat data dari Slot " + string(confirm_slot) + "?";
	} else if (menu_mode == "end") {
	    _msg = "Yakin ingin MENGAKHIRI\npermainan sekarang?";
	}
